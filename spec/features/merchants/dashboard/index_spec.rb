...
RSpec.describe 'merchant dashboard index page' do
  ...
  describe 'Merchant Dashboard Items Ready to Ship' do
    ...
    it 'displays the invoice id next to each item as a link' do
      visit("/merchants/#{merchant1.id}/dashboard")
      within "#ii-ready-to-ship-#{invoice_item20b.id}" do
        expect(page).to have_content(invoice20.id)

        click_on "#{invoice20.id}"

        expect(current_path).to eq("/merchants/#{merchant1.id}/invoices/#{invoice20.id}")
        expect(current_path).to eq(merchant_invoice_path(merchant1.id, invoice20.id))
      end
    end
    ...
    it 'displays the invoice created at date for each item' do
      ...
      within "#ii-ready-to-ship-#{invoice_item20b.id}" do
        expect(page).to have_content(invoice20.formatted_time)
      end
    end
    ...
    it 'display the invoice items from oldest to newest' do
      ...
      within "#invoice-items-ready-to-ship" do
        expect(invoice_item11b.item.name).to appear_before(invoice_item9b.item.name)
        expect(invoice_item11a.item.name).to appear_before(invoice_item7b.item.name)
      end
    end
  end
end
